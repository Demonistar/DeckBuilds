...
DECK_TEMPLATE = '''\
# generated by deck_builder_modular
...
def _remove_future_annotations_import(code: Optional[str]) -> str:
    """
    Remove duplicate `from __future__ import annotations` lines from injected
    code fragments. The generated file declares this import once in the global
    runtime header.
    """
    if not code:
        return ""
    ...
    try:
        ...
        for mod_key, mod_data in MODULES.items():
            ...
            if mod_key in selected_modules:
                ...
                if code:
                    code = _remove_future_annotations_import(code)
        ...
        deck_impl = _remove_future_annotations_import(_get_deck_implementation(log_fn))
