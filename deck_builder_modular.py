...
""",
    ...
    source = _replace_once(
        ...
        "\n"
        "        # Left sphere = active reserve from runtime token pool\n"
        "        left_orb_fill = min(1.0, self._token_count / 4096.0)\n"
        "        if self._left_orb is not None:\n"
        "            self._left_orb.setFill(left_orb_fill, available=True)\n"
        "\n"
        "        # Right sphere = VRAM availability\n"
        "        if self._right_orb is not None:\n"
        "            if NVML_OK and gpu_handle:\n"
        "                try:\n"
        "                    mem = pynvml.nvmlDeviceGetMemoryInfo(gpu_handle)\n"
        "                    vram_used = mem.used  / 1024**3\n"
        "                    vram_tot  = mem.total / 1024**3\n"
        "                    right_orb_fill = max(0.0, 1.0 - (vram_used / vram_tot))\n"
        "                    self._right_orb.setFill(right_orb_fill, available=True)\n"
        "                except Exception:\n"
        "                    self._right_orb.setFill(0.0, available=False)\n"
        "            else:\n"
        "                self._right_orb.setFill(0.0, available=False)\n"
        "\n"
        "        # Primary essence = inverse of left sphere fill\n"
        "        essence_primary_ratio = 1.0 - left_orb_fill\n"
        "        self._essence_primary_gauge.setValue(essence_primary_ratio * 100, f\"{essence_primary_ratio*100:.0f}%\")\n"
        "\n"
        "        # Secondary essence = RAM free\n"
        "        if PSUTIL_OK:\n"
        "            try:\n"
        "                mem       = psutil.virtual_memory()\n"
        "                essence_secondary_ratio  = 1.0 - (mem.used / mem.total)\n"
        "                self._essence_secondary_gauge.setValue(\n"
        "                    essence_secondary_ratio * 100, f\"{essence_secondary_ratio*100:.0f}%\"\n"
        "                )\n"
        "            except Exception:\n"
        "                self._essence_secondary_gauge.setUnavailable()\n"
        "        else:\n"
        "            self._essence_secondary_gauge.setUnavailable()\n",
        "        self._hw_panel.set_status_labels(\n"
        "            self._status,\n"
        ...
        "        self._essence_primary_gauge.setValue(essence_primary_ratio * 100, f\"{essence_primary_ratio*100:.0f}%\")\n"
        "\n"
        "        # Secondary essence = VRAM free (fallback to RAM free)\n"
        "        if right_orb_available:\n"
        "            self._essence_secondary_gauge.setValue(\n"
        "                right_orb_fill * 100, f\"{right_orb_fill*100:.0f}%\"\n"
        "            )\n"
        "        elif PSUTIL_OK:\n"
        "            try:\n"
        "                mem = psutil.virtual_memory()\n"
        ...
        "        else:\n"
        "            self._essence_secondary_gauge.setUnavailable()\n",
        "essence update path resilient + vram-secondary fallback",
    )
    source = _replace_once(
        source,
